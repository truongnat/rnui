...
describe("Button", () => {
  it("renders label", () => {
    const { getByText } = render(<Wrap><Button label="Save" onPress={() => { }} /></Wrap>);
  ...
  it("calls onPress when tapped", () => {
    ...
    const { getByText } = render(<Wrap><Button label="Tap" onPress={onPress} /></Wrap>);
  ...
  it("renders all variants without crashing", () => {
    ...
    variants.forEach((variant) => {
      const { getByText } = render(
        <Wrap><Button label={variant} variant={variant} onPress={() => { }} /></Wrap>
      );
...
describe("Input", () => {
  it("renders label and placeholder", () => {
    const { getByText, getByPlaceholderText } = render(
      <Wrap><Input label="Email" placeholder="you@example.com" /></Wrap>
    );
  ...
  it("shows error text", () => {
    const { getByText } = render(
      <Wrap><Input label="Email" error="Invalid email" /></Wrap>
    );
  ...
  it("shows helper text when no error", () => {
    const { getByText } = render(
      <Wrap><Input label="Email" helperText="Used for login" /></Wrap>
    );
  ...
  it("hides helper when error is present", () => {
    const { queryByText } = render(
      <Wrap><Input label="Email" error="Required" helperText="Used for login" /></Wrap>
    );
  ...
  it("fires onChangeText", () => {
    ...
    const { getByPlaceholderText } = render(
      <Wrap><Input placeholder="type" onChangeText={onChange} /></Wrap>
    );
...
describe("Checkbox", () => {
  it("renders label", () => {
    const { getByText } = render(<Wrap><Checkbox label="Accept terms" /></Wrap>);
  ...
  it("calls onChange on press", () => {
    const onChange = jest.fn();
    const { getByText } = render(
      <Wrap><Checkbox label="Accept" onChange={onChange} /></Wrap>
    );
  ...
  it("does not call onChange when disabled", () => {
    const onChange = jest.fn();
    const { getByText } = render(
      <Wrap><Checkbox label="Accept" disabled onChange={onChange} /></Wrap>
    );
    fireEvent.press(getByText("Accept"));
...
describe("Switch", () => {
  it("renders label and calls onChange", () => {
    const onChange = jest.fn();
    const { getByText } = render(
      <Wrap><Switch label="Notifications" onChange={onChange} /></Wrap>
    );
...
describe("Badge", () => {
  it("renders label", () => {
    const { getByText } = render(<Wrap><Badge label="New" /></Wrap>);
  ...
  it("renders all variants without crashing", () => {
    ...
    variants.forEach((variant) => {
      const { getByText } = render(<Wrap><Badge label={variant} variant={variant} /></Wrap>);
      expect(getByText(variant)).toBeTruthy();
    });
  });
});
...
describe("Divider", () => {
  it("renders without label", () => {
    const { toJSON } = render(<Wrap><Divider /></Wrap>);
    expect(toJSON()).toBeTruthy();
  });

  it("renders with label", () => {
    const { getByText } = render(<Wrap><Divider label="OR" /></Wrap>);
...
describe("EmptyState", () => {
  it("renders title and description", () => {
    const { getByText } = render(
      <Wrap>
        <EmptyState title="Nothing here" description="Add your first item to get started." />
      </Wrap>
    );
...
describe("Skeleton", () => {
  it("renders without crashing", () => {
    const { toJSON } = render(<Wrap><Skeleton width={200} height={20} /></Wrap>);
    expect(toJSON()).toBeTruthy();
  });
