...
export interface TabBarItemProps<T = string> {
  value: T;
  label: string;
...
export function TabBar<T = string>({
  ...
  return (
    <TabBarContext.Provider
      value={contextValue as unknown as TabBarContextValue<unknown>}
    >
      <View
        style={[
          styles.container,
...
export function TabBarItem<T = string>({
  value,
  label,
  icon,
  activeIcon,
  ...
  const itemProps = ctx ? ctx.getItemProps(value, disabled) : ({} as any);
