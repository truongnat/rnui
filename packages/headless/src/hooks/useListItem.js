...
function useListItem({
  ...
  const close = (0, react_1.useCallback)(() => {
    translateX.value = (0, react_native_reanimated_1.withSpring)(
      0,
      snappySpring
    );
  ...
  const tapGesture = react_native_gesture_handler_1.Gesture.Tap()
    ...
    .onEnd((_, success) => {
      ...
      if (onPress) (0, react_native_worklets_1.scheduleOnRN)(onPress);
    });
  const longPressGesture = react_native_gesture_handler_1.Gesture.LongPress()
    ...
    .onStart(() => {
      'worklet';
      if (onLongPress) (0, react_native_worklets_1.scheduleOnRN)(onLongPress);
    });
  // ── Pan gesture ──────────────────────────────────────────────
  ...
  const gesture = react_native_gesture_handler_1.Gesture.Simultaneous(
    react_native_gesture_handler_1.Gesture.Race(panGesture, tapGesture),
    longPressGesture
  );
  // ── Animated styles ──────────────────────────────────────────
