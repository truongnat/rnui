...
function useBottomSheet({
  ...
  const translateY = (0, react_native_reanimated_1.useSharedValue)(
    SCREEN_HEIGHT
  );
  ...
  // ── Animate to snap ──────────────────────────────────────────
  const animateToSnap = (0, react_1.useCallback)(
    (index, onDone) => {
      'worklet';
      const targetHeight =
        snapPoints[index] ?? snapPoints[snapPoints.length - 1];
      const targetY = SCREEN_HEIGHT - targetHeight;
      translateY.value = (0, react_native_reanimated_1.withSpring)(
        targetY,
        gentleSpring,
        (finished) => {
          if (finished && onDone)
      ...
      backdropOpacity.value = (0, react_native_reanimated_1.withTiming)(
  ...
  const close = (0, react_1.useCallback)(() => {
    ...
    backdropOpacity.value = (0, react_native_reanimated_1.withTiming)(0, {
      duration: 200,
    });
  }, [translateY, backdropOpacity, handleCloseEnd, gentleSpring]);
  const snapTo = (0, react_1.useCallback)(
    (index) => {
