...
function useScrollHeader({ headerMaxHeight, headerMinHeight }) {
  ...
  const headerStyle = (0, react_native_reanimated_1.useAnimatedStyle)(() => {
    const height = (0, react_native_reanimated_1.interpolate)(
      scrollY.value,
      [0, scrollDistance],
  ...
  const imageStyle = (0, react_native_reanimated_1.useAnimatedStyle)(() => {
    const translateY = (0, react_native_reanimated_1.interpolate)(
      scrollY.value,
      [-headerMaxHeight, 0, scrollDistance],
      [-headerMaxHeight / 2, 0, scrollDistance * 0.5], // Moves at half speed relative to scroll
      react_native_reanimated_1.Extrapolation.CLAMP
    );
    const scale = (0, react_native_reanimated_1.interpolate)(
      scrollY.value,
      [-headerMaxHeight, 0],
      [2, 1],
      {
        extrapolateLeft: react_native_reanimated_1.Extrapolation.EXTEND,
        extrapolateRight: react_native_reanimated_1.Extrapolation.CLAMP,
      }
    );
    return {
  ...
  const titleStyle = (0, react_native_reanimated_1.useAnimatedStyle)(() => {
    const opacity = (0, react_native_reanimated_1.interpolate)(
      scrollY.value,
