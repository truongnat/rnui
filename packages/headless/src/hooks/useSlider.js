...
function useSlider({
  ...
  const onTrackLayout = (0, react_1.useCallback)(
    (width) => {
  ...
  const emitChange = (0, react_1.useCallback)(
    (ratio) => {
  ...
  const emitChangeEnd = (0, react_1.useCallback)(
    (ratio) => {
      const raw = ratio * (max - min) + min;
      const snapped = snapToStep(raw, min, max, step);
      onChangeEnd?.(snapped);
  ...
  const lastEmittedValue = (0, react_native_reanimated_1.useSharedValue)(
    currentValue
  );
  const panGesture = react_native_gesture_handler_1.Gesture.Pan()
    ...
    .onStart(() => {
      ...
      thumbScale.value = (0, react_native_reanimated_1.withSpring)(
        1.2,
        snappySpringConfig
      );
    ...
    .onEnd(() => {
      ...
      thumbRatio.value = (0, react_native_reanimated_1.withSpring)(
        targetRatio,
        snappySpringConfig
      );
  ...
  const thumbAnimatedStyle = (0, react_native_reanimated_1.useAnimatedStyle)(
    () => {
      ...
      return {
        transform: [{ translateX: ratio * width }, { scale: scale }],
      };
    }
  );
