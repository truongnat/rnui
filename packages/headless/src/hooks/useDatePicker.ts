...
export function useDatePicker(options: UseDatePickerOptions = {}): UseDatePickerReturn {
  ...
  const [selectedDate, setSelectedDateState] = useState<Date | null>(defaultDate || null);
  const [selectedRange, setSelectedRangeState] = useState<[Date, Date] | null>(null);
  const [displayDate, setDisplayDate] = useState<Date>(defaultDate || new Date());
  ...
  const setSelectedRange = useCallback((rangeVal: [Date, Date] | null) => {
    setSelectedRangeState(rangeVal);
    if (rangeVal) setDisplayDate(rangeVal[0]);
    onDateChange?.(rangeVal);
  }, [onDateChange]);

  const applyPreset = useCallback((preset: DatePickerPreset) => {
