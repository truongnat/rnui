...
describe('usePagination', () => {
  ...
  it('should respect custom defaultPage', () => {
    const { result } = renderHook(() =>
      usePagination({ count: 10, defaultPage: 5 })
    );
    expect(result.current.page).toBe(5);
    expect(result.current.items).toEqual([
      1,
  ...
  it('should operate in controlled mode', () => {
    ...
    const { result, rerender } = renderHook(
      (props: { count: number; page: number; onChange: jest.Mock }) =>
        usePagination(props),
  ...
  it('should handle custom siblingCount', () => {
    const { result } = renderHook(() =>
      usePagination({ count: 10, defaultPage: 5, siblingCount: 2 })
    );
    ...
    expect(result0.current.items).toEqual([
      1,
      'start-ellipsis',
      5,
      'end-ellipsis',
  ...
  it('should handle custom boundaryCount', () => {
    const { result } = renderHook(() =>
      usePagination({ count: 10, defaultPage: 5, boundaryCount: 2 })
    );
