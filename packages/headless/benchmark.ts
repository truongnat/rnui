...
function optimizedApproach(selected: any[], options: any[], placeholder: string) {
  if (!selected || (Array.isArray(selected) && selected.length === 0)) return placeholder;
  if (Array.isArray(selected)) {
...
const options = Array.from({ length: optionsCount }, (_, i) => ({ value: `val_${i}`, label: `Label ${i}` }));
const selected = Array.from({ length: selectedCount }, (_, i) => `val_${Math.floor(Math.random() * optionsCount)}`);
...
console.log(`Improvement: ${((currentAges - optimizedAges) / currentAges * 100).toFixed(2)}%`);
