...
describe("Checkbox", () => {
  it("toggles checked state on tap", async () => {
    ...
    await expect(element(by.id("checkbox-agree"))).toHaveAccessibilityValue({ checked: "true" });
    await element(by.id("checkbox-agree")).tap();
    await expect(element(by.id("checkbox-agree"))).toHaveAccessibilityValue({ checked: "false" });
  });

  it("disabled checkbox does not toggle", async () => {
    await element(by.id("checkbox-disabled")).tap();
    await expect(element(by.id("checkbox-disabled"))).toHaveAccessibilityValue({ checked: "true" });
  });
});
...
describe("Switch", () => {
  it("toggles on/off", async () => {
    ...
    await expect(element(by.id("switch-notifications"))).toHaveAccessibilityValue({ checked: "false" });
    await element(by.id("switch-notifications")).tap();
    await expect(element(by.id("switch-notifications"))).toHaveAccessibilityValue({ checked: "true" });
  });
});
...
describe("Toast", () => {
  ...
  it("persistent toast stays until manually dismissed", async () => {
    ...
    await expect(element(by.text("Processing in background…"))).not.toBeVisible();
  });
});
