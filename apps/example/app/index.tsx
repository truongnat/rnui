...
export default function KitchenSink() {
    ...
    ({ item, index }: ListRenderItemInfo<Contact>) => (
      ...
      >
        ...
        <View style={{ flex: 1, marginLeft: listItemGap }}>
          <Text style={{ fontWeight: "600", color: listPrimaryColor }}>{item.name}</Text>
          <Text style={{ fontSize: 13, color: listSecondaryColor }}>{item.role}</Text>
        </View>
      </ListItem>
    ),
  ...
  const dataHeaderRight =
    mainTab === "data" ? (
      <PillHeaderButton label={listEditMode ? "Done" : "Edit"} onPress={() => setListEditMode((v) => !v)} />
    ) : null;

  const componentsHeaderLeft = <PillHeaderButton label="UI Kit" onPress={() => toast.info("RNUI showcase")} />;

  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: t.color.bg.subtle }} edges={["top"]}>
      <ScreenHeader
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : "height"}>
        {mainTab === "components" && (
          <ScrollView
            style={{ flex: 1 }}
            contentContainerStyle={{ padding: t.spacing[4], gap: t.spacing[4], paddingBottom: scrollBottomPad }}
          ...
          >
            <Section title="Buttons">
              ...
              <Text style={{ fontSize: t.fontSize.sm, color: t.color.text.tertiary, marginTop: 4 }}>
                Full-width (Telegram-style CTA)
              </Text>
              <Button label="Start messaging" fullWidth onPress={() => toast.success("Primary action")} />
              <Button
              ...
              <FormGroup gap="md">
                ...
                <FormField label="Username" helperText="Pick a unique handle">
                  <Input
                    ...
                    trailingElement={
                      <Text style={{ color: t.color.text.tertiary, fontSize: 12 }}>@gmail.com</Text>
                    }
                  />
                </FormField>
            ...
            >
              <FormGroup
                variant="grouped"
                error={groupedEmailField.touched ? groupedEmailField.error : undefined}
            ...
            <Section title="Tabs (component demo)">
              ...
              <Text style={{ paddingVertical: 8, color: t.color.text.primary, fontSize: 14 }}>
                Active tab: {tabsDemo}
              </Text>
            </Section>
          </ScrollView>
        ...
        {mainTab === "data" && (
          <ScrollView
            ...
            keyboardShouldPersistTaps="handled"
          >
            <PillSearchBar value={dataSearch} onChangeText={setDataSearch} placeholder="Search chats" />

            <Section title="Chat list (Telegram-style rows)" flush>
              <View style={{ paddingHorizontal: t.spacing[3], paddingTop: t.spacing[2] }}>
                <SegmentedControl
                ...
                {CONTACTS.map((c, i) => (
                  <View key={c.id}>
                    <ListItem
                      ...
                      divider={false}
                      style={{ paddingVertical: 10, paddingHorizontal: t.spacing[3] }}
                    >
                      {listEditMode ? (
                        <View
                          style={{
                      ...
                      <View style={{ flex: 1, marginLeft: t.spacing[3] }}>
                        <Text style={{ fontWeight: "700", fontSize: 16, color: t.color.text.primary }}>{c.name}</Text>
                        <Text
                          style={{ fontSize: 14, color: t.color.text.secondary, marginTop: 2 }}
                          numberOfLines={1}
                        >
                          {c.role}
                        </Text>
                      ...
                      <View style={{ alignItems: "flex-end", gap: 6 }}>
                        <Text style={{ fontSize: 13, color: t.color.text.tertiary }}>{c.time}</Text>
                        <UnreadBadge count={c.unread} />
                      </View>
                    </ListItem>
                    {i < CONTACTS.length - 1 ? <GroupDivider inset={AVATAR_INSET} /> : null}
                  </View>
                ))}
              </View>
            </Section>
            ...
            <Section title="Avatar">
              ...
              <AvatarGroup size="sm" max={4} avatars={CONTACTS.map((c) => ({ initials: c.initials }))} />
            </Section>
            ...
            <Section title="Badges">
              ...
              <Text style={{ fontSize: t.fontSize.sm, color: t.color.text.tertiary }}>Unread-style (brand pill)</Text>
            ...
            <Section title="Empty State">
              <View style={{ alignItems: "center", paddingVertical: t.spacing[6], gap: t.spacing[4] }}>
                <EmptyState variant="search" size="sm" />
                <Typography variant="caption" color="tertiary" style={{ textAlign: "center", paddingHorizontal: 24 }}>
                  Recent items will appear here. Use the button below for a custom block.
                </Typography>
                <Button label="Show custom empty" variant="outline" size="sm" onPress={() => setShowEmpty((p) => !p)} />
              </View>
              <View style={{ gap: 12 }}>
              ...
              {showEmpty && (
                <EmptyState
                  ...
                  action={
                    <Button label="Clear filters" variant="outline" size="sm" onPress={() => setShowEmpty(false)} />
                  }
                />
              )}
            </Section>
            ...
            <Section title="List (grouped)">
              <View style={{ borderRadius: t.radius.lg, overflow: "hidden", backgroundColor: t.color.surface.raised }}>
                <Typography variant="h6" style={{ padding: t.spacing[3], color: t.color.text.primary }}>
                  Team
                </Typography>
                {CONTACTS.map((c, i) => (
                  <View key={c.id}>
                ...
                <Carousel
                  ...
                  renderItem={(item) => (
                    <View style={{ flex: 1, backgroundColor: item, alignItems: "center", justifyContent: "center" }}>
                      <Text style={{ color: t.color.text.inverse, fontWeight: "700", fontSize: 16 }}>Slide</Text>
                    </View>
                  )}
                />
              </View>
                ...
                <Button
                  ...
                  onPress={() =>
                    toast.show({
                      ...
                      action: { label: "Undo", onPress: () => toast.info("Restored!") },
                    })
                  }
                />
              </View>
            ...
            <Section title="Overlays">
              <View style={{ gap: 14 }}>
                <Tooltip title="Tooltip — tap the label">
                  <Text style={{ color: t.color.brand.default, fontWeight: "600", fontSize: 15 }}>Tap for tooltip</Text>
                </Tooltip>
                ...
                <Menu open={menuOpen} onClose={() => setMenuOpen(false)} anchorEl={menuAnchor}>
                  <MenuItem
                    onPress={() => {
                  ...
                  <MenuItem onPress={() => setMenuOpen(false)}>Action two</MenuItem>
                </Menu>

                <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 12, alignItems: "center" }}>
                  <Fab icon={<Plus color="#FFFFFF" />} onPress={() => toast.info("FAB tapped")} />
                  <Fab
                    variant="extended"
                    label="Create"
                ...
                <Button label="Open modal" variant="outline" size="sm" onPress={() => setDemoModalOpen(true)} />
                <Modal open={demoModalOpen} onClose={() => setDemoModalOpen(false)}>
                  <Text style={{ color: t.color.text.primary, fontSize: 18, fontWeight: "600", marginBottom: 8 }}>
                    Demo modal
                  </Text>
                  <Text style={{ color: t.color.text.secondary, marginBottom: 16, fontSize: 14 }}>
                    Modal body — tap backdrop to dismiss.
                  </Text>
                  <Button label="Close" onPress={() => setDemoModalOpen(false)} />
                </Modal>
                ...
                <Alert severity="info">
                  ...
                  <Text style={{ color: t.color.text.secondary, fontSize: 14 }}>Inline alert with title.</Text>
                </Alert>
                <Alert severity="success" variant="outlined">
                  <Text style={{ color: t.color.text.primary, fontSize: 14 }}>Success — outlined variant.</Text>
                </Alert>
                <Alert severity="warning" variant="filled">
                  <Text style={{ color: "#FFFFFF", fontSize: 14 }}>Warning — filled variant.</Text>
                </Alert>
              </View>
            </Section>
          </ScrollView>
        ...
        {mainTab === "settings" && (
          <ScrollView
            ...
            keyboardShouldPersistTaps="handled"
          >
            <Section title="Appearance" footer="Theme follows device when System is selected.">
              <View style={{ gap: 10 }}>
                <Text style={{ fontSize: t.fontSize.sm, fontWeight: "600", color: t.color.text.secondary }}>
                  Color scheme
                </Text>
                <View style={row}>
                  <Button
                ...
                <View key={rowItem.label}>
                  ...
                  >
                    ...
                    {"value" in rowItem && rowItem.value ? (
                      <Text style={{ fontSize: t.fontSize.md, color: t.color.text.tertiary, marginRight: 4 }}>
                        {rowItem.value}
                      </Text>
                    ) : null}
                    <ChevronRight size={18} color={t.color.text.tertiary} />
                  </Pressable>
                  {i < arr.length - 1 ? <GroupDivider inset={16 + 32 + 12} /> : null}
                </View>
              ))}
            </Section>
      ...
      >
        <BottomNavigation value={mainTab} onChange={(v) => setMainTab(v as MainTab)} showLabels>
          <BottomNavigationAction
            value="components"
            label="Components"
            icon={<Layers size={22} color={mainTab === "components" ? t.color.brand.default : t.color.text.tertiary} />}
          />
          <BottomNavigationAction
            value="data"
            label="Data"
            icon={<Database size={22} color={mainTab === "data" ? t.color.brand.default : t.color.text.tertiary} />}
          />
          <BottomNavigationAction
            value="feedback"
            label="Feedback"
            icon={<Bell size={22} color={mainTab === "feedback" ? t.color.brand.default : t.color.text.tertiary} />}
          />
          <BottomNavigationAction
            value="settings"
            label="Settings"
            icon={<Settings size={22} color={mainTab === "settings" ? t.color.brand.default : t.color.text.tertiary} />}
          />
        </BottomNavigation>
      </View>
      ...
      <BottomSheet ref={sheetRef} snapPoints={["50%", "90%"]} onClose={() => {}}>
        <View style={{ padding: 24, gap: 12 }}>
          <Text style={{ color: t.color.text.primary, fontSize: 18, fontWeight: "600" }}>Bottom Sheet</Text>
      ...
      <BottomSheet ref={actionSheetRef} snapPoints={["42%"]} onClose={() => {}}>
        <View style={{ paddingHorizontal: t.spacing[4], paddingTop: t.spacing[2], gap: 0 }}>
          <Text
            style={{
              textAlign: "center",
          ...
          >
            ...
            >
              <Text style={{ fontSize: 17, fontWeight: "600", color: t.color.error.text }}>End video chat</Text>
            </Pressable>
            <View style={{ height: StyleSheet.hairlineWidth, backgroundColor: t.color.border.subtle }} />
            <Pressable
              onPress={() => {
                actionSheetRef.current?.close();
              ...
              style={{ paddingVertical: 16, alignItems: "center" }}
            >
              <Text style={{ fontSize: 17, fontWeight: "600", color: t.color.brand.default }}>Leave video chat</Text>
            </Pressable>
          </View>
          <Pressable
          ...
          >
            <Text style={{ fontSize: 17, fontWeight: "600", color: t.color.brand.default }}>Cancel</Text>
          </Pressable>
        </View>
      </BottomSheet>

      <Modal open={confirmTelegramOpen} onClose={() => setConfirmTelegramOpen(false)}>
        <View style={{ alignItems: "center", gap: t.spacing[3], width: "100%" }}>
          <Text
            style={{
              fontSize: 28,
              fontWeight: "700",
          ...
          <Text style={{ fontSize: t.fontSize.md, color: t.color.text.secondary, textAlign: "center" }}>
            Is this the correct number for your account?
          </Text>
          <Pressable onPress={() => toast.info("Edit number")} feedbackMode="scaleSubtle">
            <Text style={{ fontSize: t.fontSize.md, fontWeight: "600", color: t.color.brand.default }}>Edit</Text>
          </Pressable>
          <Button label="Continue" fullWidth onPress={() => setConfirmTelegramOpen(false)} />
          <Button label="Not now" variant="ghost" fullWidth onPress={() => setConfirmTelegramOpen(false)} />
        </View>
      </Modal>
